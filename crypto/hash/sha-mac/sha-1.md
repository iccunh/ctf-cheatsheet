...
Referensi: [https://github.com/stephenbradshaw/hlextend/blob/master/hlextend.py](https://github.com/stephenbradshaw/hlextend/blob/master/hlextend.py) CSGS 2025
...
r.sendlineafter(b"animal? ", b'test')

r.recvuntil(b"token: ")
token = r.readline().decode().strip('\n')
print("token", token)

def exploit_token(orig_token, key_length):
    decoded = base64.b64decode(orig_token)
    token_part, mac_hex = decoded.split(b"|mac=")
    print("decoded", token_part, mac_hex)

    s = sha1()

    addition = b"|admin=true"
    new_msg = s.extend(addition, token_part, key_length, mac_hex.decode())
    new_mac  = s.hexdigest().encode()  

    forged = new_msg + b"|mac=" + new_mac            # data' + mac' (di luar append)
    final_token_b64 = b64e(forged)
    return final_token_b64

new_token = exploit_token(token, 32)
print("new token", new_token)
print("Token yang dipalsukan:", b64d(new_token))
