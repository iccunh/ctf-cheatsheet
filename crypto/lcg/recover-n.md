---
description: 'Referensi: ChatGPT & DawgCTF 2025'
---
...
for i in range(10):
    ...
    r.sendlineafter(b"name: ", str(i).encode())
    r.recvuntil(b"is:  ")
    state = int(r.readline().decode().strip())
    outputs.append(state)

# Recover modulus N via GCD of (x[i+1]-x[i])*(x[i+3]-x[i+2]) - (x[i+2]-x[i+1])^2
ds = []
for i in range(len(outputs)-3):
    d1 = outputs[i+1] - outputs[i]
    d2 = outputs[i+2] - outputs[i+1]
    d3 = outputs[i+3] - outputs[i+2]
    ds.append(d1 * d3 - d2 * d2)
N = abs(ds[0])
for d in ds[1:]:
    N = gcd(N, abs(d))
...
a, b, seed0 = params_from_three(outputs[0],outputs[1],outputs[2] ,N)

print("a,b,seed", a,b ,seed0)

r.sendlineafter(b"> ", str(2).encode())
r.sendlineafter(b"name: ", b"admin")
r.sendlineafter(b"password: ", str(seed0).encode())
r.interactive()
